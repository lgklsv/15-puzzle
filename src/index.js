...
function getEmptyTile(colIndex, rowIndex) {
...
function checkWin() {
  for (let i = 0; i < state.gameSize; i++) {
    for (let j = 0; j < state.gameSize; j++) {
      if (field[i][j] !== winArr[i][j]) return false;
    }
  }
...
function showGameWinScreen(winTime) {
  if (canvas.width == 350) {
    ...
    context.fillText(`Hooray! 🎉🎉🎉`, 175, 60);
    context.fillText(`You solved the puzzle in`, 175, 100);
    context.fillText(`${formatSeconds(winTime)}`, 175, 140);
    context.fillText(`and ${moves} moves!`, 175, 180);
    context.fillText(`Click to continue 🔁`, 175, 250);
  }
  if (canvas.width == 300) {
    context.font = '24px Roboto';
    context.fillStyle = '#3c3c48';
    context.textAlign = 'center';
    context.fillText(`Hooray! 🎉🎉🎉`, 150, 40);
    context.fillText(`You solved the puzzle in`, 150, 80);
    context.fillText(`${formatSeconds(winTime)}`, 150, 120);
    context.fillText(`and ${moves} moves!`, 150, 160);
    context.fillText(`Click to continue 🔁`, 150, 230);
  }
  if (canvas.width == 400) {
...
function getResultsByTime(size) {
  return state.results
    .filter((resobj) => resobj.size == size)
    .sort((a, b) => a.time - b.time)
    .slice(0, 10)
    .map((resObj) => generateResult(resObj))
    .join('');
}
...
function swipeDetect(el) {
  ...
  surface.addEventListener('mouseup', function (e) {
    ...
    if (emptyTile) {
      ...
      if (
        x > emptyTileCoords.x &&
        x < emptyTileCoords.x + tileSize &&
        y > emptyTileCoords.y &&
        y < emptyTileCoords.y + tileSize
      ) {
        const temp = field[colIndex][rowIndex];
        field[colIndex][rowIndex] = 0;
        field[emptyTile.col][emptyTile.row] = temp;
